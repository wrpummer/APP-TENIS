-- Rode este arquivo no SQL Editor do Supabase para corrigir a funcao
-- public.refresh_season_derived_data antes de executar o showcase demo.
...
as $$
begin
  delete from public.season_rankings where season_id = target_season_id and imported_from_legacy = false;
  ...
  insert into public.hall_of_fame (season_id, category, player_id, value_text, value_number)
  select target_season_id, category, player_id, value_text, value_number
  from (
    select
      'champion'::text as category,
      ranked.player_id,
      'Maior pontuacao da temporada'::text as value_text,
      ranked.points::numeric as value_number
    from (
      select
        rs.player_id,
        rs.points,
        row_number() over (order by rs.points desc, rs.wins desc, rs.player_id) as rn
      from tmp_ranked_season rs
    ) ranked
    where ranked.rn = 1
    ...
    from (
      select
        rs.player_id,
        rs.wins,
        row_number() over (order by rs.wins desc, rs.points desc, rs.player_id) as rn
      from tmp_ranked_season rs
    ) ranked
    where ranked.rn = 1
    ...
    from (
      select
        ...
        rs.win_rate,
        rs.matches_played,
        row_number() over (order by rs.win_rate desc, rs.matches_played desc, rs.player_id) as rn
      from tmp_ranked_season rs
      where rs.matches_played > 0
    ) ranked
    where ranked.rn = 1
    ...
    select
      ...
      ranked.matches_played::numeric
    from (
      select
        rs.player_id,
        rs.matches_played,
        rs.points,
        row_number() over (order by rs.matches_played desc, rs.points desc, rs.player_id) as rn
      from tmp_ranked_season rs
    ) ranked
    where ranked.rn = 1
  ) hall_rows;
